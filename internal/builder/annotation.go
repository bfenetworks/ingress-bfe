...
const (
	...
	LoadBalanceWeightKey        = "balance.weight"
	LoadBalanceWeightAnnotation = BfeAnnotationPrefix + LoadBalanceWeightKey
...
// Check validates cookie annotation string with form "key: value"
...
// Check validates header annotation string with form "key: value"
...
func BuildBfeAnnotations(k8sAnnotations map[string]string) []BfeAnnotation {
	...
	// build annotations
	for key, value := range k8sAnnotations {
		bfeAnnotation, err := BuildBfeAnnotation(key, value)
		if err == nil {
			bfeAnnotations = append(bfeAnnotations, bfeAnnotation)
		}
	}

	// sort annotations
	SortAnnotations(bfeAnnotations)
	return bfeAnnotations
}

// BuildBfeAnnotation build BFE annotation by key & value
